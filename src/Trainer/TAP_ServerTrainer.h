#ifndef _TAPSERVERTRAINER_
#define _TAPSERVERTRAINER_
...
class TAP_ServerTrainer : public Trainer{
public:

    TAP_ServerTrainer(Model *model, Datapoint *datapoint) : Trainer(model,datapoint){}
    ...
    void Train() override{
            ...
            for(int inter_counter = 0 ; inter_counter < in_inter; ){
                ...
                while(flag_receive){
                    ...
                    if(delay_counter[taskid - 1] > max_delay){
                        ...
                        // mat master_weight = model->GetWeight();
                    ...
                    // model->Update(learning_rate, worker_gradient);
        ...
        fstream outfile("../data/result/nuclear_lasso_tap_10.txt", ios::out);

        int size = weight_data_list.size();
        for(int i = 0; i < size; i++){
            master_weight = weight_data_list.back();
            model->SetWeight(master_weight);
